...
def adverse_event_analysis(
    ...
    results = {"analysis_type": "Comprehensive Adverse Event Analysis"}
    ...
    for treatment in data[treatment_col].unique():
        ...
        summary = {
            ...
            "ae_rate_per_subject": (
                total_aes / n_subjects_with_ae if n_subjects_with_ae > 0 else 0
            ),
        }
        ...
        if exposure_col and exposure_col in trt_data.columns:
            ...
            summary["ae_rate_per_exposure"] = (
                total_aes / total_exposure if total_exposure > 0 else 0
            )
    ...
    ae_by_term = (
        data.groupby([treatment_col, ae_term_col])
        .agg({subject_col: "nunique", ae_term_col: "count"})
    ...
    if severity_col and severity_col in data.columns:
        severity_analysis = (
            data.groupby([treatment_col, severity_col])
            .agg({subject_col: "nunique", severity_col: "count"})
    ...
    if soc_col and soc_col in data.columns:
        soc_analysis = (
            data.groupby([treatment_col, soc_col])
            .agg({subject_col: "nunique", soc_col: "count"})
...
def laboratory_shift_analysis(
    ...
    results = {"analysis_type": "Laboratory Shift Analysis"}

    # Create shift combinations
    data["shift_category"] = (
        data[baseline_col].astype(str) + " → " + data[post_baseline_col].astype(str)
    )
    ...
    shift_summary = (
        data.groupby([treatment_col, "shift_category"])
        .agg({subject_col: "nunique"})
    ...
    shift_summary["percentage"] = shift_summary.apply(
        lambda row: (
            (row["n_subjects"] / total_by_treatment[row.name[0]]) * 100
            if row.name[0] in total_by_treatment
            else 0
        ),
        axis=1,
    )
...
def exposure_analysis(
    ...
    for treatment in data[treatment_col].unique():
        ...
        exposure_categories = {
            ...
            f"30-89 {exposure_unit}": (
                (trt_data[exposure_col] >= 30) & (trt_data[exposure_col] < 90)
            ).sum(),
            f"90-179 {exposure_unit}": (
                (trt_data[exposure_col] >= 90) & (trt_data[exposure_col] < 180)
            ).sum(),
