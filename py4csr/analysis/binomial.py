...
def binomial_proportion_ci(
    successes: int, trials: int, confidence_level: float = 0.95, method: str = "wilson"
) -> Dict:
    """
    ...
    lower_ci, upper_ci = proportion_confint(
        successes, trials, alpha=alpha, method=method
    )
...
def binomial_test(
    ...
    p_value = stats.binom_test(
        successes, trials, expected_prob, alternative=alternative
    )
...
def proportion_difference_test(
    data: pd.DataFrame,
    group_var: str,
    response_var: str,
    group1: str,
