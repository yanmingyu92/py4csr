        ...
        for stratum in data[strata].unique():
            ...
            if len(stratum_data) > 0:
                contingency_table = pd.crosstab(
                    stratum_data[row_var], stratum_data[col_var]
                )
...
def fisher_exact_test(
    data: pd.DataFrame, row_var: str, col_var: str, alternative: str = "two-sided"
) -> Dict:
    """
    ...
    odds_ratio, p_value = fisher_exact(
        contingency_table.values, alternative=alternative
    )
...
def cochran_mantel_haenszel_test(
    data: pd.DataFrame, row_var: str, col_var: str, strata: str
) -> Dict:
    """
...
def mcnemar_test(data: pd.DataFrame, var1: str, var2: str, exact: bool = True) -> Dict:
    """
