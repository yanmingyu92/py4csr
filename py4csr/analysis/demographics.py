        ...
        if var in categorical_vars:
            ...
            for category in sorted(data[var].dropna().unique()):
                ...
                if include_total:
                    ...
                    pct_total = (
                        (n_category_total / n_total_total * 100)
                        if n_total_total > 0
                        else 0
                    )
...
def _process_categorical_variable(
    data: pd.DataFrame,
    var: str,
    treatment_var: str,
    treatment_groups: List[str],
    test_statistics: bool,
) -> List[List]:
    ...
    percentages = (
        pd.crosstab(data[var], data[treatment_var], normalize="columns", dropna=False)
        * 100
    )
...
def _process_continuous_variable(
    data: pd.DataFrame,
    var: str,
    treatment_var: str,
    treatment_groups: List[str],
    ...
    stats_by_group = (
        data.groupby(treatment_var)[var].agg(["count", "mean", "std"]).round(2)
    )
    ...
    if test_statistics:
        ...
        try:
            groups_data = [
                data[data[treatment_var] == grp][var].dropna()
                for grp in treatment_groups
            ]
            groups_data = [
                grp for grp in groups_data if len(grp) > 0
            ]  # Remove empty groups
    ...
    # Median and range
    median_stats = (
        data.groupby(treatment_var)[var].agg(["median", "min", "max"]).round(1)
    )
