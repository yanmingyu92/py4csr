...
def conditional_odds_ratio(
    ...
    try:
        ...
        se_coef = (
            np.sqrt(np.diag(np.linalg.inv(X.T @ X)))[0]
        ...
        warnings.warn(
            "sklearn not available, using stratified analysis as approximation"
        )
        return stratified_odds_ratio(
            data,
            exposure_var,
            outcome_var,
            conditioning_vars[0] if conditioning_vars else None,
            confidence_level,
        )
    except Exception as e:
        return {"error": str(e), "method": "Conditional Odds Ratio"}


def stratified_odds_ratio(
    data: pd.DataFrame,
    exposure_var: str,
    outcome_var: str,
    ...
    if strata_var is None:
        ...
        return odds_ratio_analysis(
            data, exposure_var, outcome_var, None, confidence_level
        )

    # Use the existing stratified logic from odds_ratio_analysis
    return odds_ratio_analysis(
        data, exposure_var, outcome_var, strata_var, confidence_level
    )
