...
def create_disposition_table(
    data, treatment_var="TRT01P", disposition_vars=["SAFFL", "EFFFL", "DTHFL"]
):
    """
...
def create_population_summary(
    data, treatment_var="TRT01P", population_flags=["SAFFL", "EFFFL"]
):
    """
...
def _format_population_name(flag_name: str) -> str:
    ...
    return name_mapping.get(
        flag_name, flag_name.replace("FL", "").replace("_", " ").title()
    )


def analyze_discontinuations(
    data: pd.DataFrame, treatment_var: str, discontinuation_var: str = "DCSREAS"
) -> pd.DataFrame:
    """
    ...
    disc_counts = (
        discontinued.groupby([discontinuation_var, treatment_var])
        .size()
    ...
    disc_summary["percentage"] = (
        disc_summary["count"] / disc_summary["total"] * 100
    ).round(1)
    disc_summary["formatted"] = (
        disc_summary["count"].astype(str)
        + " ("
        + disc_summary["percentage"].astype(str)
        + "%)"
    )
    ...
    discontinuation_table = (
        disc_summary.pivot(
            index=discontinuation_var, columns=treatment_var, values="formatted"
        )
        .fillna("0 (0.0%)")
        .reset_index()
    )
