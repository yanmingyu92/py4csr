...
def create_ae_summary(
    ...
    treatment_var: str,
    term_var: str,
    severity_var: Optional[str] = None,
) -> pd.DataFrame:
    """
    ...
    subject_counts = (
        data.groupby(treatment_var)["USUBJID"]
        .nunique()
        .reset_index(name="total_subjects")
    )

    # Merge and calculate percentages
    ae_summary = ae_counts.merge(subject_counts, on=treatment_var)
    ae_summary["percentage"] = (
        ae_summary["count"] / ae_summary["total_subjects"] * 100
    ).round(1)
    ae_summary["formatted"] = (
        ae_summary["count"].astype(str)
        + " ("
        + ae_summary["percentage"].astype(str)
        + "%)"
    )

    # Pivot to wide format
    summary_table = ae_summary.pivot(
        index=term_var, columns=treatment_var, values="formatted"
    ).fillna("0 (0.0%)")
...
def create_ae_specific_table(
    data, treatment_var="TRT01A", term_var="AEDECOD", system_var="AEBODSYS"
):
    """
    ...
    for soc in sorted(data[system_var].unique()):
        ...
        for trt in treatments:
            ...
            soc_row[f"{trt}_pct"] = (
                f'({n_subjects/len(data[data[treatment_var]==trt]["USUBJID"].unique())*100:.1f})'
            )
...
def create_lab_summary(
    ...
    lab_stats = (
        data.groupby([parameter_var, visit_var, treatment_var])[value_var]
    ...
    # Format mean (SD)
    lab_stats["mean_sd"] = (
        lab_stats["mean"].astype(str) + " (" + lab_stats["std"].astype(str) + ")"
    )

    # Format median (min, max)
    lab_stats["median_range"] = (
        lab_stats["median"].astype(str)
        + " ("
