...
def format_mean_sd(
    mean: float, sd: float, mean_digits: int = 1, sd_digits: int = 2
) -> str:
    """
...
def format_pvalue(
    p_value: float, digits: int = 3, threshold: Optional[float] = None
) -> str:
    """
...
def format_number(
    value: Union[float, int], digits: int = 1, width: Optional[int] = None
) -> str:
    """
...
def calculate_locf(
    ...
    locf_values = (
        data_sorted.groupby(subject_col)[value_col].fillna(method="ffill").values
    )
