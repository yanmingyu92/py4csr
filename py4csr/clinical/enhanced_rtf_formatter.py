...
class EnhancedClinicalRTFFormatter:
    ...
    def _get_main_table(
        ...
        # Extract treatment columns (exclude Parameter column)
        # The parameter column is the first column (could be "Parameter" or custom name)
        param_col = (
            table_data.columns[0] if len(table_data.columns) > 0 else "Parameter"
        )
        ...
        if len(treatment_cols) > 0:
            ...
            trt_width = remaining_width // (
                len(treatment_cols) + 1
            )  # +1 for p-value column
        ...
        col_defs.append(
            f"\\clvertalt\\clbrdrt\\brdrs\\brdrw10 \\clbrdrl\\brdrnone \\clbrdrb\\brdrs\\brdrw10 \\clbrdrr\\brdrnone \\cltxlrtb\\clftsWidth3\\clwWidth{param_width}\\clshdrawnil \\cellx{current_x}"
        )

        for i, col in enumerate(treatment_cols):
            current_x += trt_width
            col_defs.append(
                f"\\clvertalt\\clbrdrt\\brdrs\\brdrw10 \\clbrdrl\\brdrnone \\clbrdrb\\brdrs\\brdrw10 \\clbrdrr\\brdrnone \\cltxlrtb\\clftsWidth3\\clwWidth{trt_width}\\clshdrawnil \\cellx{current_x}"
            )

        # P-value column
        current_x += pval_width
        col_defs.append(
            f"\\clvertalt\\clbrdrt\\brdrs\\brdrw10 \\clbrdrl\\brdrnone \\clbrdrb\\brdrs\\brdrw10 \\clbrdrr\\brdrnone \\cltxlrtb\\clftsWidth3\\clwWidth{pval_width}\\clshdrawnil \\cellx{current_x}"
        )
        ...
        for col in treatment_cols:
            # Check if this is a shift table column (contains '|' separator)
            if "|" in col:
                # Shift table column - split on '|' and format as two lines
                parts = col.split("|")
                line1 = parts[0].strip()
                ...
                header_rtf += (
                    "\\pard \\ltrpar\\qc \\li0\\ri0\\sl220\\slmult0\\nowidctlpar\\intbl\\wrapdefault\\faauto\\rin0\\lin0 "
                    f"{{\\rtlch\\fcs1 \\af0\\afs18 \\ltrch\\fcs0 \\fs18 {line1}\\par }}"
                    "\\pard \\ltrpar\\qc \\li0\\ri0\\sa80\\sl220\\slmult0\\nowidctlpar\\intbl\\wrapdefault\\faauto\\rin0\\lin0 "
                    f"{{\\rtlch\\fcs1 \\af0\\afs18 \\ltrch\\fcs0 \\fs18 {line2}\\cell }}"
                )
            # Check if column name already contains (N=XX) format
            elif "\n(N=" in col or "(N=" in col:
                # Column already has N count - use as is, split into two lines
                if "\n" in col:
                    parts = col.split("\n")
                    trt_name = parts[0]
                ...
                header_rtf += (
                    ...
                    "\\pard \\ltrpar\\qc \\li0\\ri0\\sa80\\sl220\\slmult0\\nowidctlpar\\intbl\\wrapdefault\\faauto\\rin0\\lin0 "
                    f"{{\\rtlch\\fcs1 \\af0\\afs18 \\ltrch\\fcs0 \\fs18 {n_part}\\cell }}"
                )
            else:
                # Column doesn't have N count - add it from treatment_info
                trt_name = col
                n_count = "?"
        ...
        for idx, row in table_data.iterrows():
            ...
            row_col_defs = col_def_str.replace(
                "\\clbrdrt\\brdrs\\brdrw10", "\\clbrdrt\\brdrnone"
            ).replace("\\clbrdrb\\brdrs\\brdrw10", "\\clbrdrb\\brdrnone")
