...
class DataProcessor:
    ...
    def clean_data(
        self, data: pd.DataFrame, cleaning_rules: Dict[str, Any]
    ) -> pd.DataFrame:
        ...
        cleaned_data = reduce(
            lambda df, func: func(df, cleaning_rules), cleaning_functions, cleaned_data
        )

        return cleaned_data

    def transform_data(
        self, data: pd.DataFrame, transformations: List[Callable]
    ) -> pd.DataFrame:
