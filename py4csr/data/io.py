...
# Optional dependency for SAS file reading
try:
    import pyreadstat
    HAS_PYREADSTAT = True
except ImportError:
...
def read_sas(
    filepath: Union[str, Path], encoding: Optional[str] = None, **kwargs
) -> pd.DataFrame:
    """
...
def load_dataset(
    filepath: Union[str, Path], dataset_type: Optional[str] = None, **kwargs
) -> pd.DataFrame:
    """
...
def read_xpt(file_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        raise ImportError(
            "pyreadstat package is required to read XPT files. Install with: pip install pyreadstat"
        )
    except Exception as e:
