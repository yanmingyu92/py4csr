...
class AdvancedUtilities:
    ...
    def advanced_string_manipulation(
        data: pd.DataFrame, operations: List[Dict]
    ) -> pd.DataFrame:
        """
        ...
        for op in operations:
            ...
            if operation == "standardize_case":
                ...
                result_data[column] = result_data[column].str.replace(
                    r"\s+", " ", regex=True
                )
    ...
    @staticmethod
    def advanced_date_processing(
        data: pd.DataFrame, date_operations: List[Dict]
    ) -> pd.DataFrame:
        """
        ...
        for op in date_operations:
            ...
            if not pd.api.types.is_datetime64_any_dtype(result_data[column]):
                result_data[column] = pd.to_datetime(
                    result_data[column], errors="coerce"
                )
            ...
            if operation == "extract_components":
                ...
                if end_column and end_column in result_data.columns:
                    if not pd.api.types.is_datetime64_any_dtype(
                        result_data[end_column]
                    ):
                        result_data[end_column] = pd.to_datetime(
                            result_data[end_column], errors="coerce"
                        )
                    ...
                    if unit == "days":
                        ...
                        result_data[duration_column] = (duration.dt.days / 30.44).round(
                            1
                        )
                    elif unit == "years":
                        result_data[duration_column] = (
                            duration.dt.days / 365.25
                        ).round(1)

        return result_data

    @staticmethod
    def advanced_numeric_formatting(
        data: pd.DataFrame, formatting_rules: Dict[str, Dict]
    ) -> pd.DataFrame:
        """
