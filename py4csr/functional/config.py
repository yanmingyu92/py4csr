...
class FunctionalConfig:
    ...
    decimal_precision: Dict[str, int] = field(
    ...
    def clinical_standard(cls) -> "FunctionalConfig":
        ...
        config.templates = {
            ...
            "disposition": TableTemplate(
                ...
                footnotes=["Percentages based on number of randomized subjects"],
            ),
            ...
            "survival": TableTemplate(
                name="survival",
                title_template="Time-to-Event Analysis (Kaplan–Meier)",
                subtitle_template="{study_title} - {population_label}",
                default_statistics=["median"],
                footnotes=[
                    "Median survival estimated via Kaplan–Meier method",
            ...
            "efficacy": TableTemplate(
                ...
                footnotes=["Analysis based on efficacy analysis population"],
            ),
        }
    ...
    def from_yaml(cls, config_path: Union[str, Path]) -> "FunctionalConfig":
        ...
        for key, value in config_data.items():
            if key not in ["statistics", "formats", "templates"] and hasattr(
                config, key
            ):
    ...
    def validate(self) -> List[str]:
        ...
        for stat_name, stat in self.statistics.items():
            if stat.format_function and stat.format_function not in self.formats:
                issues.append(
                    f"Statistic '{stat_name}' references unknown format '{stat.format_function}'"
                )
        ...
        for tmpl_name, tmpl in self.templates.items():
            for stat_name in tmpl.default_statistics:
                if stat_name not in self.statistics:
                    issues.append(
                        f"Template '{tmpl_name}' references unknown statistic '{stat_name}'"
                    )
