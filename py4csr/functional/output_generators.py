...
class RTFGenerator(BaseOutputGenerator):
    ...
    def _generate_rtf_table(self, data: pd.DataFrame) -> List[str]:
        ...
        for i in range(num_cols):
            ...
            rtf_lines.append(
                f"\\clbrdrt\\brdrs\\clbrdrl\\brdrs\\clbrdrb\\brdrs\\clbrdrr\\brdrs\\cellx{cell_right}"
            )
        ...
        try:
            ...
            if table_result.footnotes:
                ...
                footnote_style = ParagraphStyle(
                    "Footnote", parent=styles["Normal"], fontSize=8, spaceAfter=6
                )
            ...
            if not table_result.data.empty:
                ...
                for _, row in table_result.data.iterrows():
                    for col_idx, value in enumerate(row, 1):
                        cell = ws.cell(
                            row=current_row, column=col_idx, value=str(value)
                        )
                        cell.border = Border(
                            top=Side(style="thin"),
                            bottom=Side(style="thin"),
