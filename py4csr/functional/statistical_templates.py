...
class StatisticalTemplates:
        ...
        for treatment in sorted(clean_data[treatment_var].unique()):
            ...
            for category in all_categories:
                ...
                for stat_name in statistics:
                    ...
                    stat_row["VALUE"] = value
                    stat_row["FORMATTED_VALUE"] = self._format_statistic(
                        value, stat_def
                    )
                    results.append(stat_row)
        ...
        for category in all_categories:
            ...
            for stat_name in statistics:
                ...
                if stat_name == "n":
                    ...
                    value = (
                        (cat_count / overall_total * 100) if overall_total > 0 else 0
                    )
                elif stat_name == "n_percent":
                    percent = (
                        (cat_count / overall_total * 100) if overall_total > 0 else 0
    ...
    def calculate_ae_statistics(
        self,
        data: pd.DataFrame,
        treatment_var: str,
        soc_var: str = "AEBODSYS",
        pt_var: str = "AEDECOD",
    ) -> pd.DataFrame:
        """
