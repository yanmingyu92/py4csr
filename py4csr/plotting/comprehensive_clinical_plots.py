...
try:
    from py4csr.plotting.sas_compatible_rtf_generator import (
        _generate_rtf_for_plot_professional,
    )

    try:
        from py4csr.plotting.sas_compatible_rtf_generator import (
            _generate_enhanced_forest_rtf,
...
class EnhancedPlotConfig:
    ...
    output_formats: List[OutputFormat] = field(
        default_factory=lambda: [OutputFormat.PNG, OutputFormat.HTML]
    )
    interactive_features: List[InteractiveFeature] = field(
...
def validate_sas_parameters(
    sas_params: FDACompliantPlotParameters, plot_type: SASMacroType
) -> Dict[str, Any]:
    """
    ...
    if not sas_params.protocol:
        validation_results["recommendations"].append(
            "Protocol parameter recommended for FDA submissions"
        )

    if not any([sas_params.title1, sas_params.title2, sas_params.title3]):
        validation_results["recommendations"].append(
            "At least one title recommended for FDA submissions"
        )
...
def parse_sas_parameter_string(
    param_string: str, delimiter: str = "~"
) -> List[Dict[str, str]]:
    """
...
def calculate_fda_statistics(
    data: pd.DataFrame, value_col: str, group_col: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    if group_col:
        ...
        for group_name, group_data in grouped:
            ...
            if group_stats["n"] > 1:
                ...
                ci = stats.t.interval(
                    0.95, group_stats["n"] - 1, group_stats["mean"], se
                )
        ...
        if len(clean_data) > 1:
            ...
            ci = stats.t.interval(
                0.95, len(clean_data) - 1, stats_dict["overall"]["mean"], se
            )
...
def create_fda_dual_output_boxplot(config: EnhancedPlotConfig) -> PlotOutput:
    ...
    if not x_col:
        x_col = find_column_robust(
            config.data.columns, ["AVISITN", "VISIT", "VISITNUM", "TIME", "TIMEPOINT"]
        )
    if not y_col:
        y_col = find_column_robust(
            config.data.columns, ["AVAL", "CHG", "PCHG", "VALUE", "RESULT"]
        )
    if not group_col:
        group_col = find_column_robust(
            config.data.columns, ["TRT01P", "TRTP", "ARM", "TREATMENT", "GROUP"]
        )
    ...
    if not required_cols:
        print(
            "⚠️ Error: Could not identify required columns. Please specify x_col and y_col explicitly."
        )
        return PlotOutput()
        ...
        for i, group in enumerate(config.data[group_col].unique()):
            ...
            interactive_fig.add_trace(
                go.Box(
                    ...
                    marker_color=get_fda_clinical_colors(
                        config.data[group_col].nunique(), interactive=True
                    )[i],
                    showlegend=True,
                )
            )
    ...
    for i in range(1, 7):
        ...
        title_val = getattr(sas_params, title_attr, None) or getattr(
            config, title_attr, None
        )
        if title_val:
    ...
    output = PlotOutput(
        ...
        metadata={
            ...
            "columns_used": {"x_col": x_col, "y_col": y_col, "group_col": group_col},
        },
    )

    # Add SAS compliance report
    if config.fda_compliant:
        output.sas_compliance_report = validate_sas_parameters(
            sas_params, SASMacroType.GBOX2
        )
    ...
    if sas_params.blockvarn and sas_params.blockvarc and sas_params.blockValCol:
        try:
            ...
            if isinstance(sas_params.blockvarn, str) and "~" in sas_params.blockvarn:
                block_values = [
                    float(x.strip()) for x in sas_params.blockvarn.split("~")
                ]
            else:
                block_values = (
                    [float(sas_params.blockvarn)] if sas_params.blockvarn else []
                )
                ...
                try:
                    ...
                    for val in unique_x_values:
                        try:
                            ...
                            print(
                                f"⚠️ Warning: Period backgrounds require numeric x-axis values. Found string values in '{x_col}'"
                            )
                            print(
                                f"   Hint: Use ClassVarn (numeric) instead of ClassVarc (character) for period support"
                            )
                            return output
                    ...
                    x_value_to_position = {
                        val: i + 1 for i, val in enumerate(unique_x_values)
                    }

                    # Add period backgrounds with correct positioning
                    periods_with_backgrounds = (
                        0  # Track actual colored backgrounds added
                    )
                    for i, block_val in enumerate(block_values):
                        if i < len(block_specs) and "color" in block_specs[i]:
                            ...
                            ax.axvspan(
                                start_pos,
                                end_pos,
                                alpha=block_transparency,
                                color=color,
                                zorder=0,
                            )
                            periods_with_backgrounds += (
                                1  # Increment only when background is actually added
                            )
                    ...
                    if periods_with_backgrounds > 0:
                        print(
                            f"✅ Successfully added {periods_with_backgrounds} colored period backgrounds"
                        )
                        if periods_with_backgrounds < len(block_values):
                            periods_without_color = (
                                len(block_values) - periods_with_backgrounds
                            )
                            print(
                                f"   Note: {periods_without_color} period(s) had no color specified"
                            )
                    else:
                        print(
                            f"⚠️ No colored backgrounds added - no colors specified in blockValCol"
                        )
                        print(f"   Total periods defined: {len(block_values)}")

                except Exception as conversion_error:
                    print(
                        f"⚠️ Warning: Could not process x-axis values for period backgrounds: {conversion_error}"
                    )
                    print(
                        f"   Available x-values: {list(unique_x_values)[:5]}..."
                    )  # Show first 5 values
            else:
                print(
                    f"⚠️ Warning: X-axis column '{x_col}' not found for period backgrounds"
                )
...
def create_fda_dual_output_lineplot(config: EnhancedPlotConfig) -> PlotOutput:
        ...
        try:
            ...
            for i, (block_val, block_spec) in enumerate(zip(block_values, block_specs)):
                if "color" in block_spec:
                    ...
                    # Add background rectangle
                    ax.axvspan(
                        x_start, x_end, alpha=block_transparency, color=color, zorder=0
                    )
    ...
    if group_col and len(config.data[group_col].unique()) > 0:
        ...
        for i, group in enumerate(config.data[group_col].unique()):
            ...
            try:
                group_summary = (
                    group_data.groupby(x_col)[y_col]
                    .agg(["mean", "std", "count"])
                    .reset_index()
                )
                ...
                if (
                    config.show_ci
                    and sas_params.ErrBarLowVar
                    and sas_params.ErrBarHighVar
                ):
                    ...
                    ax.errorbar(
                        ...
                        alpha=0.5,
                        capsize=3,
                        capthick=sas_params.ErrBarthickness,
                    )
            except Exception as e:
        ...
        try:
            if x_col in config.data.columns:
                data_summary = (
                    config.data.groupby(x_col)[y_col]
                    .agg(["mean", "std", "count"])
                    .reset_index()
                )
    ...
    if sas_params.Ylabel:
        ax.set_ylabel(
            sas_params.Ylabel, fontsize=sas_params.YlabelSize or sas_params.Ylabelsize
        )
    elif config.y_label:
        ax.set_ylabel(config.y_label)
        ...
        if group_col and len(config.data[group_col].unique()) > 0:
            for i, group in enumerate(config.data[group_col].unique()):
                ...
                if config.show_ci:
                    ...
                    interactive_fig.add_trace(
                        go.Scatter(
                            x=group_summary[x_col].tolist()
                            + group_summary[x_col].tolist()[::-1],
                            y=(group_summary["mean"] + 1.96 * se).tolist()
                            + (group_summary["mean"] - 1.96 * se).tolist()[::-1],
                            fill="tonexty",
            ...
            if x_col in config.data.columns:
                ...
                if len(data_summary) > 0:
                    ...
                    if config.show_ci:
                        ...
                        interactive_fig.add_trace(
                            go.Scatter(
                                x=data_summary[x_col].tolist()
                                + data_summary[x_col].tolist()[::-1],
                                y=(data_summary["mean"] + 1.96 * se).tolist()
                                + (data_summary["mean"] - 1.96 * se).tolist()[::-1],
                                fill="tonexty",
    ...
    if config.fda_compliant:
        output.sas_compliance_report = validate_sas_parameters(
            sas_params, SASMacroType.GLINE2
        )

    return output


def clinical_forest_plot(
    config: Union[PlotConfig, EnhancedPlotConfig],
    ...
    if not (lower_col and upper_col):
        ax.text(
            0.5,
            0.5,
            "Error: No confidence interval columns found\n(Expected: CI_LOWER/CI_UPPER, LOWER/UPPER, etc.)",
            ha="center",
            va="center",
            transform=ax.transAxes,
            color="red",
    ...
    q_statistic = np.sum(weights * (estimates - overall_estimate_fixed) ** 2) * np.sum(
        weights
    )
    df = n_studies - 1
    ...
    if n_studies >= 3:
        try:
            ...
            # Linear regression
            slope, intercept, r_value, egger_p, std_err = stats.linregress(
    ...
    is_ratio = estimate_col.upper() in [
        "HR",
        "OR",
        "HAZARD_RATIO",
        "ODDS_RATIO",
    ...
    y_positions = list(range(n_studies + 3))[
        ::-1
    ]  # +3 for overall, prediction interval, and spacing

    # Plot individual studies with enhanced visualization
    colors = get_clinical_colors(1)
    study_color = colors[0]

    for i, (estimate, lower, upper, label, y_pos) in enumerate(
        zip(estimates, ci_lower, ci_upper, labels, y_positions[3:])
    ):
        # Plot confidence interval line with caps
        ax.plot([lower, upper], [y_pos, y_pos], "k-", linewidth=2, alpha=0.7)
        ax.plot(
            [lower, lower], [y_pos - 0.1, y_pos + 0.1], "k-", linewidth=2, alpha=0.7
        )  # Left cap
        ax.plot(
            [upper, upper], [y_pos - 0.1, y_pos + 0.1], "k-", linewidth=2, alpha=0.7
        )  # Right cap
    ...
    z_score = (
        overall_est / overall_se_random
        if i_squared > 25
        else overall_est / overall_se_fixed
    )
    ...
    ax.axvline(
        x=ref_value, color="black", linestyle="-", alpha=0.8, linewidth=2, zorder=1
    )

    # Add "No Effect" label
    ax.text(
        ref_value,
        np.max(y_positions) + 0.5,
        "No Effect",
        ha="center",
        va="bottom",
        fontsize=11,
    ...
    if hasattr(config, "output_formats") and config.output_formats:
        if (
            OutputFormat.RTF in config.output_formats
            and OutputFormat.PNG not in config.output_formats
        ):
    ...
    if config.show_stats and n_studies > 1:
        ...
        stats_parts.append(
            f"Q = {q_statistic:.2f} (df={df}), p = {p_heterogeneity:.3f}"
        )

        if egger_p is not None:
            if egger_p < 0.05:
                stats_parts.append(
                    f"Egger's test: p = {egger_p:.3f} (significant bias)"
                )
            else:
                stats_parts.append(
                    f"Egger's test: p = {egger_p:.3f} (no significant bias)"
                )
...
def quick_boxplot(
    data: pd.DataFrame, x_col: str, y_col: str, group_col: str = None, **kwargs
) -> Tuple[plt.Figure, plt.Axes]:
...
def quick_km_plot(
    data: pd.DataFrame, time_col: str, event_col: str, group_col: str = None, **kwargs
) -> Tuple[plt.Figure, plt.Axes]:
    """Quick Kaplan-Meier plot for LLM agents (legacy)"""
    config = PlotConfig(
        data=data,
        plot_type=PlotType.KM,
        x_col=time_col,
        y_col=event_col,
        group_col=group_col,
        **kwargs,
...
def quick_forest_plot(
    ...
    **kwargs,
    ...
    config = PlotConfig(
        data=data, plot_type=PlotType.FOREST, y_col=estimate_col, **kwargs
    )
    return clinical_forest_plot(config)


# Legacy functions for backward compatibility
def _create_clinical_lineplot(
    data, x_col, y_col, group_col=None, subject_col=None, **kwargs
):
...
def create_dual_output_km_plot(
    config: Union[PlotConfig, EnhancedPlotConfig],
) -> PlotOutput:
    ...
    try:
            ...
            for i, group in enumerate(groups):
                ...
                fig_plotly.add_trace(
                    go.Scatter(
                        ...
                        line=dict(
                            color=colors[i], width=3, shape="hv"
                        ),  # hv for step function
                ...
                if config.show_ci:
                    ...
                    fig_plotly.add_trace(
                        go.Scatter(
                            x=np.concatenate(
                                [km_result["times"], km_result["times"][::-1]]
                            ),
        ...
        fig_plotly.update_yaxes(
            title_text="", showticklabels=False, gridcolor="rgba(0,0,0,0)", row=2, col=1
        )
        ...
        if (
            hasattr(config, "include_validation_info")
            and config.include_validation_info
        ):
            output.validation_results = {
                "data_quality": {
                    ...
                    "negative_times": (
                        (data[time_col] < 0).sum() if time_col in data.columns else 0
                    ),
                },
                "survival_analysis": {
                    "valid_event_coding": (
                        data[event_col].isin([0, 1]).all()
...
def create_dual_output_forest_plot(
    config: Union[PlotConfig, EnhancedPlotConfig],
) -> PlotOutput:
    ...
    try:
        ...
        if estimate_col and lower_col and upper_col:
            ...
            fig_plotly.add_trace(
                go.Scatter(
                    ...
                    marker=dict(
                        size=[
                            12 + (w / 10 if weights is not None else 0)
                            for w in (
                                weights
                                if weights is not None
                                else [10] * len(estimates)
                            )
                        ],
                        color="#1f77b4",
                        symbol="diamond",
            ...
            if is_ratio:
                x_range = max(estimates.max(), upper_ci.max()) - min(
                    estimates.min(), lower_ci.min()
                )
                left_pos = (
                    ref_value * 0.5
                    if ref_value > 0
                    else estimates.min() - 0.1 * x_range
                )
                right_pos = (
                    ref_value * 1.5
                    if ref_value > 0
                    else estimates.max() + 0.1 * x_range
                )
        ...
        if (
            ...
            output.validation_results = {
                "data_quality": {
                    "complete_cases": (
                        min_length if "min_length" in locals() else len(data)
                    ),
                    "missing_estimates": (
                        data[estimate_col].isna().sum() if estimate_col else 0
                    ),
                    "missing_ci": (
                        (data[lower_col].isna().sum() + data[upper_col].isna().sum())
                ...
                "statistical_validity": {
                    "valid_confidence_intervals": (
                        ((lower_ci <= estimates) & (estimates <= upper_ci)).all()
                        if estimate_col and lower_col and upper_col
                        else False
                    ),
                    "positive_estimates": (
                        (estimates > 0).all() if is_ratio and estimate_col else True
                    ),
                },
            }
...
def create_dual_output_boxplot(
    config: Union[PlotConfig, EnhancedPlotConfig],
) -> PlotOutput:
...
def create_dual_output_lineplot(
    config: Union[PlotConfig, EnhancedPlotConfig],
) -> PlotOutput:
...
def save_dual_outputs(
    output: PlotOutput, config: Union[PlotConfig, EnhancedPlotConfig]
) -> Dict[str, str]:
    ...
    try:
        ...
        output_formats = getattr(
            config, "output_formats", [OutputFormat.PNG, OutputFormat.HTML]
        )

        # Save interactive plot (HTML) first
        if (
            output.interactive_figure
            and config.html_path
            and (OutputFormat.HTML in output_formats)
        ):
        ...
        if (
            config.rtf_path
            and output.static_figure
            and (OutputFormat.RTF in output_formats)
        ):
            ...
            try:
                ...
                if config.plot_type == PlotType.FOREST:
                    rtf_content = _generate_enhanced_forest_rtf(
                        plot_path=temp_png_path, config=config, output=output
                    )
                elif config.plot_type == PlotType.KM:
        ...
        if (
            output.static_figure
            and config.save_path
            and (OutputFormat.PNG in output_formats)
        ):
            output.static_figure.savefig(
                config.save_path,
                dpi=config.dpi,
...
def create_enhanced_html(
    ...
    plotly_html = pyo.plot(
        output.interactive_figure, output_type="div", include_plotlyjs=True
    )
    ...
    if output.validation_results and config.include_validation_info:
        ...
        if "statistics" in output.validation_results:
            ...
            for group, group_stats in stats.items():
                validation_html += (
                    f"<tr><td colspan='2'><strong>{group}</strong></td></tr>"
                )
                for stat_name, stat_value in group_stats.items():
                    if isinstance(stat_value, float):
                        stat_value = f"{stat_value:.3f}"
                    validation_html += (
                        f"<tr><td>{stat_name}</td><td>{stat_value}</td></tr>"
                    )
    ...
    <body>
        <div class="container">
            ...
            <div class="footer">
                <p>Generated by py4csr Clinical Plotting Engine v3.0 </p>
...
def clinical_boxplot(
    ...
    return output.static_figure, (
        output.static_figure.axes[0] if output.static_figure else (None, None)
    )


def clinical_lineplot(
    config: Union[PlotConfig, EnhancedPlotConfig],
) -> Tuple[plt.Figure, plt.Axes]:
    """Enhanced legacy function with dual output capability"""
    if isinstance(config, PlotConfig):
...
def clinical_km_plot(
    ...
    if time_col not in data.columns:
        ax_main.text(
            0.5,
            0.5,
            f'Error: Time column "{time_col}" not found in data',
            ha="center",
            va="center",
    ...
    if event_col not in data.columns:
        ax_main.text(
            0.5,
            0.5,
            f'Error: Event column "{event_col}" not found in data',
            ha="center",
            va="center",
            transform=ax_main.transAxes,
            color="red",
        ...
        for i, group in enumerate(groups):
            ...
            if config.show_ci:
                ...
                se = np.sqrt(
                    km_result["survival"]
                    * (1 - km_result["survival"])
                    / np.maximum(1, km_result["n_at_risk"])
                )
                ci_lower = np.maximum(0, km_result["survival"] - 1.96 * se)
                ci_upper = np.minimum(1, km_result["survival"] + 1.96 * se)
...
def gbox2_python(data: pd.DataFrame, **sas_params) -> PlotOutput:
    ...
    **sas_params
    ...
    config = EnhancedPlotConfig(
        data=data, plot_type=PlotType.BOX, sas_params=fda_params, fda_compliant=True
    )
...
def gline2_python(data: pd.DataFrame, **sas_params) -> PlotOutput:
    ...
    **sas_params
    ...
    # Create enhanced plot configuration
    config = EnhancedPlotConfig(
        data=data, plot_type=PlotType.LINE, sas_params=fda_params, fda_compliant=True
    )
...
def create_fda_dual_output_waterfall(config: EnhancedPlotConfig) -> PlotOutput:
    ...
    if not x_col:
        x_col = find_column_robust(
            config.data.columns, ["SUBJID", "USUBJID", "SUBJECT", "ID"]
        )
    if not y_col:
        y_col = find_column_robust(
            config.data.columns, ["AVAL", "CHG", "PCHG", "VALUE", "RESULT"]
    ...
    if sas_params.trend and sas_params.trend.lower() == "down":
        sorted_data = config.data.sort_values(y_col, ascending=False).reset_index(
            drop=True
        )
    else:
        sorted_data = config.data.sort_values(y_col, ascending=True).reset_index(
            drop=True
        )
    ...
    colors = [
        "#2ca02c" if val >= 0 else "#d62728" for val in y_values
    ]  # Green for positive, Red for negative

    bars = ax.bar(
        x_positions, y_values, color=colors, alpha=0.7, width=sas_params.barwidth or 0.8
    )
    ...
    interactive_fig.add_trace(
        go.Bar(
            ...
            marker_color=colors,
            text=(
                [f"{val:.1f}" for val in y_values]
                if convert_sas_boolean(sas_params.seglabel)
                else None
            ),
            textposition="auto",
            name="Response",
    ...
    output = PlotOutput(
        ...
        metadata={
            ...
            "data_shape": config.data.shape,
            "timestamp": datetime.now().isoformat(),
            "columns_used": {"x_col": x_col, "y_col": y_col},
        },
    )
    ...
    if config.fda_compliant:
        output.sas_compliance_report = validate_sas_parameters(
            sas_params, SASMacroType.GWATERFALL2
        )
