...
from ..analysis.utils import format_ci, format_mean_sd, format_pvalue
...
class ClinicalStudyReports:
    ...
    def _create_sample_datasets(self):
        ...
        adsl["TRT01AN"] = adsl[
            "TRT01PN"
        ]  # Actual treatment number = planned treatment number for simplicity
        ...
        adae = pd.DataFrame(
            {
                ...
                "AESEV": np.random.choice(
                    ["MILD", "MODERATE", "SEVERE"], n_ae_records, p=[0.6, 0.3, 0.1]
                ),
        ...
        adlb = pd.DataFrame(
            {
                ...
                "PARAMCD": np.random.choice(
                    ["GLUC", "ALT", "AST", "CREAT", "BUN"], n_lab_records
                ),
                ...
                "AVISIT": np.random.choice(
                    ["Baseline", "Week 12", "Week 24"], n_lab_records
                ),
        ...
        self.datasets = {"adsl": adsl, "adae": adae, "adlb": adlb}
    ...
    def generate_baseline_characteristics(self) -> str:
        ...
        # Create RTF table
        rtf_table = (
            RTFTable(demographics)
            .rtf_title(
                "Baseline Characteristics of Participants",
                "(All Participants Randomized)",
            )
            .rtf_colheader(
                "Characteristic | Category | Placebo | Xanomeline Low Dose | Xanomeline High Dose | Total | P-value",
    ...
    def generate_ae_summary(self) -> str:
        ...
        for category, label in ae_categories.items():
            if category == "any_ae":
                ...
                ae_counts = (
                    adae[adae["AEREL"].isin(["POSSIBLY RELATED", "PROBABLY RELATED"])]
                ...
                ae_counts = (
                    adae[
                        (adae["AESER"] == "Y")
                        & (adae["AEREL"].isin(["POSSIBLY RELATED", "PROBABLY RELATED"]))
                    ]
                    .groupby("TRT01A")["USUBJID"]
                    .nunique()
                    .reset_index()
                ...
                ae_counts = (
                    adae[adae["AEOUT"] == "FATAL"]
                    .groupby("TRT01A")["USUBJID"]
                    .nunique()
                    .reset_index()
                )

            ae_counts = ae_counts.merge(
                pop.rename(columns={"TRT01P": "TRT01A"}), on="TRT01A", how="right"
            )
            ae_counts["USUBJID"] = ae_counts["USUBJID"].fillna(0)
            ae_counts["pct"] = (
                ae_counts["USUBJID"] / ae_counts["n_total"] * 100
            ).round(1)
        ...
        ae_summary_table = ae_summary_df.pivot(
            index="Category", columns="Treatment", values=["n", "pct"]
        )

        # Flatten column names and format
        ae_summary_table.columns = [
            f"{col[1]}_{col[0]}" for col in ae_summary_table.columns
        ]
        ae_summary_table = ae_summary_table.reset_index()

        # Create RTF table
        rtf_table = (
            RTFTable(ae_summary_table)
            .rtf_title(
                "Analysis of Adverse Event Summary", "(Safety Analysis Population)"
            )
            .rtf_colheader(
                "Category | Placebo | | Xanomeline Low Dose | | Xanomeline High Dose | ",
                col_rel_width=[3.5, 1, 1, 1, 1, 1, 1],
            )
            .rtf_colheader(
                " | n | (%) | n | (%) | n | (%)", col_rel_width=[3.5, 1, 1, 1, 1, 1, 1]
            )
            .rtf_body(
                col_rel_width=[3.5, 1, 1, 1, 1, 1, 1],
    ...
    def generate_ae_specific(self) -> str:
        ...
        ae_specific = create_ae_specific_table(
            data=adae, treatment_var="TRT01A", term_var="AEDECOD", system_var="AEBODSYS"
        )

        # Create RTF table
        rtf_table = (
            RTFTable(ae_specific)
            .rtf_title(
    ...
    def generate_efficacy_table(self) -> str:
        ...
        ancova_results = ancova_analysis(
            data=gluc_data, endpoint="CHG", treatment="TRT01P", baseline="BASE"
        )
        ...
        # Create RTF table
        rtf_table = (
            RTFTable(efficacy_table)
            .rtf_title(
                "Analysis of Change from Baseline in Glucose at Week 24",
    ...
    def generate_disposition_table(self) -> str:
        ...
        # Create RTF table
        rtf_table = (
            RTFTable(disposition)
            .rtf_title("Subject Disposition", "(All Randomized Subjects)")
            .rtf_colheader(
                "Category | Placebo | Xanomeline Low Dose | Xanomeline High Dose",
                col_rel_width=[3, 2, 2, 2],
            )
            .rtf_body(
                col_rel_width=[3, 2, 2, 2], text_justification=["l", "c", "c", "c"]
            )
        )
    ...
    def generate_population_table(self) -> str:
        ...
        population = create_population_summary(
            data=adsl, treatment_var="TRT01P", population_flags=["SAFFL", "EFFFL"]
        )

        # Create RTF table
        rtf_table = (
            RTFTable(population)
            .rtf_title("Analysis Population Summary", "(All Randomized Subjects)")
            .rtf_colheader(
    ...
    def generate_km_plot(self) -> str:
        ...
        survival_data["EVENT"] = np.random.choice(
            [0, 1], len(survival_data), p=[0.7, 0.3]
        )
        ...
        rtf_table = (
            RTFTable(km_table)
            .rtf_title(
                "Kaplan-Meier Plot of Time to Event", "(Efficacy Analysis Population)"
            )
    ...
    def assemble_complete_report(self, tlf_files: Dict[str, str]) -> str:
        ...
        rtf_combined = (
            ...
            .rtf_colheader("TLF Type | Description | Filename", col_rel_width=[2, 4, 3])
            .rtf_body(col_rel_width=[2, 4, 3], text_justification=["c", "l", "l"])
            .rtf_footnote(
                "This report contains all standard TLFs for regulatory submission."
            )
        )
