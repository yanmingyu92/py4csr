...
class BaseTableGenerator(ABC):
    ...
    def apply_template(
        self, template_name: str, data: pd.DataFrame, **kwargs
    ) -> pd.DataFrame:
        """
    ...
    def validate_data(
        self, data: pd.DataFrame, spec: TableSpecification
    ) -> Dict[str, Any]:
        """
        ...
        if trt_var not in data.columns:
            validation_result["passed"] = False
            validation_result["errors"].append(
                f"Treatment variable '{trt_var}' not found"
            )
    ...
    def generate_metadata(
        self, spec: TableSpecification, generation_time: datetime = None
    ) -> Dict[str, Any]:
        """
    ...
    def post_process_data(
        ...
        if trt_var in data.columns:
            ...
            if (
                hasattr(spec.config, "treatments")
                and "levels" in spec.config.treatments
            ):
                treatment_order = spec.config.treatments["levels"]
                data[trt_var] = pd.Categorical(
                    data[trt_var], categories=treatment_order, ordered=True
                )
