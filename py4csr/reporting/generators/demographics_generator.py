...
class DemographicsGenerator(BaseTableGenerator):
        ...
        for var in variables:
            if var in data.columns:
                if self._is_categorical(data[var]):
                    var_summary = self._generate_categorical_summary(
                        data, var, trt_var, spec
                    )
                else:
                    var_summary = self._generate_continuous_summary(
                        data, var, trt_var, spec
                    )
    ...
    def _generate_categorical_summary(
        self, data: pd.DataFrame, var: str, trt_var: str, spec: TableSpecification
    ) -> pd.DataFrame:
        ...
        header_row = {"Variable": var_label, "Category": "", "Statistic": "n (%)"}
        ...
        for category in sorted(categories):
            ...
            row = {"Variable": "", "Category": f"  {category}", "Statistic": ""}
    ...
    def _generate_continuous_summary(
        self, data: pd.DataFrame, var: str, trt_var: str, spec: TableSpecification
    ) -> pd.DataFrame:
