...
class RTFTable:
    ...
    def rtf_encode(self) -> str:
        ...
        rtf_content.append(
            r"{\fonttbl{\f0 " + self.page_settings["font_family"] + r";}}"
        )
        ...
        if self.page_header:
            rtf_content.append(
                r"{\header\pard\qr " + self._escape_rtf(self.page_header) + r"\par}"
            )

        # Page footer
        if self.page_footer:
            rtf_content.append(
                r"{\footer\pard\qc " + self._escape_rtf(self.page_footer) + r"\par}"
            )
    ...
    def _embed_image(self, img_info: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if img_info["width"] and img_info["height"]:
                width_twips = int(
                    img_info["width"] * 15
                )  # Convert pixels to twips (approximately)
    ...
    def _generate_table(self) -> str:
        ...
        n_cols = (
            len(self.colheader)
            if self.colheader
            else len(self.body[0]) if self.body else 0
        )
        if n_cols == 0:
            return ""
