...
"""
        ...
        if subtitle:
            ...
            for line in subtitle_lines:
                header += (
                    r"""{\pard\qc\f0\fs"""
                    + str(self.header_font_size)
                    + r""" """
                    + self._escape_rtf_text(line)
                    + r"""\par}"""
                )
    ...
    def _create_table_row(
        self, row_data: List[str], col_widths: List[int], is_data_row: bool = True
    ) -> str:
    ...
    def _create_footnotes(self, footnotes: List[str]) -> str:
        ...
        for i, footnote in enumerate(footnotes):
            ...
            footnote_section += (
                r"""{\super """ + marker + r"""} """ + self._escape_rtf_text(footnote)
            )
    ...
    def _calculate_column_widths(
        self, df: pd.DataFrame, base_width: int = 1440
    ) -> List[int]:
    ...
    def format_table_to_rtf(
        ...
        for _, row in df.iterrows():
            ...
            rtf_content += self._create_table_row(
                row_data, col_widths, is_data_row=True
            )
    ...
    def format_listing_to_rtf(
        ...
        for _, row in df.iterrows():
            ...
            for val in row:
                row_data.append(
                    self._escape_rtf_text(str(val) if not pd.isna(val) else "")
                )
