...
class ClinicalTableBuilder:
    ...
    def create_demographics_table(
        ...
        # Gender distribution
        gender_stats = self._calculate_categorical_stats(
            adsl_data, "SEX", treatment_var
        )
        demo_summary.extend(gender_stats)

        # Race distribution
        if "RACE" in adsl_data.columns:
            race_stats = self._calculate_categorical_stats(
                adsl_data, "RACE", treatment_var
            )
        ...
        outputs = self._generate_table_outputs(
            df, "demographics_table", table_type="demographics", study_title=study_title
        )
    ...
    def create_adverse_events_table(
        ...
        any_ae_stats = self._calculate_ae_incidence(
            adae_data, adsl_data, treatment_var, category="Any Adverse Event"
        )
        ...
        if "AESOC" in adae_data.columns:
            ...
            for soc in top_socs:
                ...
                soc_stats = self._calculate_ae_incidence(
                    soc_data, adsl_data, treatment_var, category=f"{soc}ᵃ"
                )
        ...
        # Generate outputs
        outputs = self._generate_table_outputs(
            df, "adverse_events_table", table_type="ae", study_title=study_title
        )
    ...
    def create_laboratory_table(
        ...
        for param in parameters:
            ...
            if not param_data.empty:
                param_stats = self._calculate_continuous_stats(
                    param_data, value_var, treatment_var, parameter_name=param
                )
        ...
        # Generate outputs
        outputs = self._generate_table_outputs(
            df, "laboratory_table", table_type="lab", study_title=study_title
        )

        return outputs

    def _calculate_continuous_stats(
        self,
        data: pd.DataFrame,
        value_var: str,
        treatment_var: str,
    ...
    def _calculate_categorical_stats(
        self, data: pd.DataFrame, cat_var: str, treatment_var: str
    ) -> List[Dict]:
        ...
        for category in categories:
            cat_row = {
                "Parameter": f'{cat_var.replace("_", " ").title()}',
                "Statistic": category,
            }
    ...
    def _generate_table_outputs(
        ...
        if table_type == "demographics":
            ...
            rtf_content = self.rtf_formatter.format_table(
                df, "Clinical Summary Table", study_title
            )
    ...
    def _create_html_table(
        self, df: pd.DataFrame, table_type: str, study_title: str = None
    ) -> str:
